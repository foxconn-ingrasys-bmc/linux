...
*	2. 2017/07/21 William Lin: add dual/quad support.
*
********************************************************************************/
//#define DEBUG
...
//	printk("trl : %x \n",spi_ctrl);
	spi_ctrl &= ~(SPI_DUAL_DATA | SPI_IO_MODE_MASK);	
...
//		spi_ctrl &= ~SPI_CPOL_1;
	...
	if(nor->flash_read == SPI_NOR_DUAL){
		   ...
		   printk("Enter Quad Read Mode\n");
			...
			ulCtrlData |= (0x0b << 16);
			ulCtrlData |= (1 << 6);
			printk("Enter Fast read Mode\n");
	   }
	fmc_spi_write(host, ulCtrlData, 0x00);
#endif
	return err;
}

static void fmc_spi_config(struct fmc_spi_host *host, struct spi_device *spi, struct spi_transfer *xfer)
{
	u32 ulCtrlData;

	ulCtrlData = fmc_spi_read(host, 0x00);
	...
	else if (xfer->tx_nbits == SPI_NBITS_QUAD && xfer->rx_nbits == SPI_NBITS_QUAD){
		//ulCtrlData |= SPI_CMD_DATA(CMD_READ_QUAD_OUTPUT_FAST);
		ulCtrlData |= IOMODEx4_dummy;
...
//	writel( (readl(host->spi_data->ctrl_reg) | SPI_CMD_USER_MODE) | SPI_CE_INACTIVE,host->spi_data->ctrl_reg);
	...
	fmc_spi_write(host, (fmc_spi_read(host, 0x00) & ~SPI_CE_HIGH), 0x00);
	msg->actual_length = 0;
	msg->status = 0;
...
//	writel( SPI_CE_INACTIVE | readl(host->spi_data->ctrl_reg),host->spi_data->ctrl_reg);
	fmc_spi_write(host, (fmc_spi_read(host, 0x00) & ~SPI_CMD_USER_MODE), 0x00);
	fmc_spi_write(host, (fmc_spi_read(host, 0x00) | SPI_CE_HIGH), 0x00);
	fmc_spi_write(host, SPI_CMD_DATA(0x0b) | SPI_CLK_DIV(7) | SPI_DUMMY_LOW(1) | SPI_CMD_FAST_R_MODE, 0x00);//fast read
	msg->status = 0;
...
#if defined(CONFIG_ARCH_AST2400)
	host->master->mode_bits = SPI_MODE_0 | SPI_TX_DUAL | SPI_RX_DUAL | SPI_RX_QUAD | SPI_RX_QUAD;
#endif

#if defined(CONFIG_ARCH_AST2500)
	host->master->mode_bits = SPI_MODE_0 | SPI_TX_DUAL | SPI_RX_DUAL;
#endif
